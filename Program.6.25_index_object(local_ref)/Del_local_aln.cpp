...
int Local_aln::Judge_SV(char *Seq, uint32_t ReadLength, uint32_t kmer, uint32_t GU, uint32_t GD)
{
	...
	uint32_t	Local_ref_L = GD - GU;
	uint64_t 	SR_length = Local_ref_L + 1 - kmer;
	uint32_t 	pre = 0;
	...
	{
		pre = ((pre<<2)&mask)|trans[genome[ GU + i + kmer - 1 ]];
		tempArray[i] = pre;
		tempArrayNext[i] = pre;
	}
	...
	{
		str_array[str_array_L].read_begin = 0;
		str_array[str_array_L].ref_begin = readSRhash[k + readKmerhash[pre].Kpointer1]+GU;
		str_array_L++;
	}

	for( int i = 1 ; i < ReadLength + 1 - kmer ; i++ )
	{
		//uint32_t l2r = transfer(Seq, kmer, i);
		pre = ((pre<<2)&mask)|trans[Seq[ i + kmer - 1 ]];
		how_many_node = readKmerhash[pre].Kpointer2 - readKmerhash[pre].Kpointer1;
		for( int k = 0 ; k < how_many_node ; k++ )
		{
			str_array[str_array_L].read_begin = i;
			str_array[str_array_L].ref_begin = readSRhash[k + readKmerhash[pre].Kpointer1]+GU;
			str_array_L++;
		}
	}
	...
	for( int i = 0 ; i < str_array_L ; i++ )
	{
		out << str_array[i].ref_begin << "\t";
		out << str_array[i].read_begin << "\n";
	}
	...
	{
		Tuple temp_node;
		temp_node.ref_begin = str_array[0].ref_begin;
		temp_node.read_begin = str_array[0].read_begin;
		int local_ans = 0;
		for( int i = 1 ; i < SR_length ; i++ )
		{
			local_ans = Tuple_link( temp_node, str_array[i] );
			if( local_ans == 1 )
			{
				temp_node.ref_begin = str_array[i].ref_begin;
				temp_node.read_begin = str_array[i].read_begin;
			}
		}
...
void Local_aln::Files_open(char* path, uint32_t kmer)
{
	...
	str_array = new Tuple[READ_MAX_LENGTH];
	if (NULL == str_array) {
		fprintf(stderr,"Fail to alloc str_array, now exit");
		exit(1);
	}

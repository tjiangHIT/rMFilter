...
int main(int argc, char const *argv[])
{
	...
	FILE* inHash_1;
	inHash_1 = fopen("/home/tjiang/Hash_1","rb");
	fread(kmerHash,sizeof(uint32_t),Kmer_code_length,inHash_1);
	fclose(inHash_1);

	uint64_t Hash2sum = 248591776;//225371321;//223830061;
	/*
	file NOTE
	...
	*/
	...
	FILE* inHash_2;
	inHash_2 = fopen("/home/tjiang/Hash_2","rb");
	fread(SR_HASH,sizeof(SRdata),Hash2sum,inHash_2);
	fclose(inHash_2);

	ofstream fout;
	fout.open("../LowHITread.newtry.2.Version_bp..");

	gzFile fp;
	kseq_t *Seq;

	fp = gzopen("/home/tjiang/3rd_Version2/Chr1_test_data/_0001.fastq", "r");
	Seq = kseq_init(fp);
	...
	while (kseq_read(Seq) >= 0) {
		...
		*/
		...
		//fout << MaxScore * seed_bet;
		//fout << MaxScore;
		fout << endl;
		fout << MaxRegion.ps << "\t" << MaxRegion.pe << "\t" << MaxRegion.pf << endl;
		/*for( int lm = 0 ; lm < 1000 ; lm++ )
			fout << lm * seed_bet<< "\t" << con_array[lm] << endl;
		break;*/
		if( cycle_time == 100 ) break;
